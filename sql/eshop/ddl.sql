...
DROP PROCEDURE IF EXISTS insert_produkt_kategori;
...
ON kundorder FOR EACH ROW
    INSERT INTO logg (kundorder, loggdatum, kommentar)
        VALUES (NEW.ordernummer, NOW(), 'beställning skapad')
;
...
ON kundorder FOR EACH ROW
    INSERT INTO logg (kundorder, loggdatum, kommentar)
        VALUES (NEW.ordernummer, NOW(), 'beställning ändrad')
;
...
--
-- Procedure to show all category
--
DELIMITER ;;
...
BEGIN
    ...
    FROM
        produkt AS p
        LEFT OUTER JOIN produkt_kategori as pk
            ON p.produktkod = pk.produkt
        LEFT OUTER JOIN kategori AS k
            ON pk.kategori = k.id
    ORDER BY grund, produktnamn
    ;
END
;;
DELIMITER ;
...
--
-- Procedure to insert into produkt_kategori table
--
DELIMITER ;;
CREATE PROCEDURE insert_produkt_kategori(
    a_produktkod INT,
    a_kategoriId INT
)
BEGIN
    INSERT INTO
        produkt_kategori(produkt, kategori)
    VALUES
        (a_produktkod, a_kategoriId)
...
--
-- Procedure to delete from produkt_kategori table
--
DELIMITER ;;
CREATE PROCEDURE delete_produkt_kategori(
    a_produktkod INT,
    a_kategoriId INT
)
BEGIN
    DELETE FROM produkt_kategori
    WHERE
        produkt = a_produktkod AND
        kategori = a_kategoriId
    ;
END
;;
DELIMITER ;
...
BEGIN
    ...
    p.produktpris,
    s.antal,
    k.grund AS grundkategori,
    GROUP_CONCAT(k.typ) AS kategori
    FROM produkt AS p
    ...
    ORDER BY grundkategori, produktnamn
    ;
END
;;
DELIMITER ;
...
--
DELIMITER ;;
CREATE PROCEDURE show_product_order_id()
BEGIN
    SELECT
    p.produktkod,
    p.produktnamn,
...
--
-- Procedure to show one products categories
-- . Visa även information om vilken kategori som produkten
--
DELIMITER ;;
CREATE PROCEDURE show_products_categories(
    a_id INT
)
BEGIN
    SELECT
...
DELIMITER ;;
CREATE PROCEDURE show_new_product_code()
BEGIN
    SELECT
        Auto_increment AS new
    FROM
...
BEGIN
    SELECT
        kundorder AS Ordernummer,
        faktura AS Fakturanummer,
        DATE_FORMAT(loggdatum, "%Y-%m-%d") AS orderdatum,
...
--
-- Procedure to show stock on produktkod
--
DELIMITER ;;
CREATE PROCEDURE show_stock_on_product(
    a_produktkod INT
)
BEGIN
    SELECT
    ...
    lagerhylla AS l
    LEFT OUTER JOIN stock AS s
        ON l.hylla = s.lagerhylla
    LEFT OUTER JOIN produkt AS p
        ON p.produktkod = s.produkt
    WHERE
        p.produktkod = a_produktkod
    ;
END
;;
DELIMITER ;
...
BEGIN
    ...
    FROM produkt as p
        LEFT OUTER JOIN stock AS s
            on p.produktkod = s.produkt
    ORDER BY s.lagerhylla, p.produktnamn
    ;
END
;;
DELIMITER ;
...
-- Procedure to delete from stock (abitrary of shelf)

DELIMITER ;;
CREATE PROCEDURE remove_from_stock_arbitrary(
    a_produktkod INT,
    a_antal INT
)
BEGIN
    ...
    SET
        antal = antal - a_antal
    WHERE
        produkt = a_produktkod
    ;
END
;;
DELIMITER ;

-- Procedure to hard delete from stock

DELIMITER ;;
CREATE PROCEDURE hard_delete_stock(
    a_produktkod INT,
    a_lagerhylla INT
)
BEGIN
...
--
-- Procedure to show all picklists
--
DELIMITER ;;
CREATE PROCEDURE show_all_plocklista()
BEGIN
    SELECT
        k.ordernummer,
        DATE_FORMAT(k.orderdatum, "%Y-%m-%d") AS orderdatum,
        k.kund,
        GROUP_CONCAT(kr.produkt) AS produkt
    FROM kundorder AS k
        JOIN kundorder_rad AS kr
            ON k.ordernummer = kr.kundorder
        JOIN stock AS s
            ON kr.produkt = s.produkt
    WHERE
        -- Vill inte göra om till picklist om den inte är beställd än
        k.orderdatum IS NOT NULL AND
        k.skickad IS NULL AND
        k.borttagen IS NULL
    GROUP BY
        k.ordernummer
    ORDER BY
...
BEGIN
    SELECT
        ...
        k.kund,
        kr.produkt,
        kr.antal AS antal_beställda,
        s.lagerhylla,
        s.antal AS antal_lager
    FROM kundorder AS k
        JOIN kundorder_rad AS kr
            ON k.ordernummer = kr.kundorder
        JOIN stock AS s
            ON kr.produkt = s.produkt
    WHERE
        k.ordernummer = a_order_number AND
        -- Vill inte göra om till picklist om den inte är beställd än eller om den redan är skickad
        k.orderdatum IS NOT NULL AND
        k.skickad IS NULL AND
        k.borttagen IS NULL
    ORDER BY
...
BEGIN
    ...
    AND skickad IS NULL;
    SELECT
        ordernummer AS Ordernummer,
        kund AS Kund,
        DATE_FORMAT(orderdatum, "%Y-%m-%d") AS Orderdatum,
        DATE_FORMAT(uppdaterad, "%Y-%m-%d") AS Uppdaterad,
        DATE_FORMAT(skickad, "%Y-%m-%d") AS Skickad
    FROM kundorder
    WHERE ordernummer = a_order_number;
END
;;
...
--
-- Procedure to remove from stock by ordernummer
--
