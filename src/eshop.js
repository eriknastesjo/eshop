...
module.exports = {
    ...
    showProductOrderId: showProductOrderId,
    showProductkod: showProductkod,
    showProductkodCategories: showProductkodCategories,
    getNewProductkod: getNewProductkod,
    editProduct: editProduct,
    createProduct: createProduct,
    deleteProduct: deleteProduct,
    showLog: showLog,
    showShelfs: showShelfs,
    showStockOnProduct: showStockOnProduct,
    showInv: showInv,
    searchInv: searchInv,
    addToShelf: addToShelf,
    removeFromShelf: removeFromShelf,
    hardDeleteFromStock: hardDeleteFromStock
};
...
async function insertProductCategory(produktKod, kategoriId) {
    let sql = `CALL insert_produkt_kategori(?, ?);`;
...
async function deleteProductCategory(produktKod, kategoriId) {
    let sql = `CALL delete_produkt_kategori(?, ?);`;
...
async function showProductOrderId() {
    let sql = `CALL show_product_order_id();`;
    let res;
 ...
 * @returns {RowDataPacket} Resultset from the query.
 */
async function showProductkodCategories(produktkod) {
    let sql = `CALL show_products_categories(?);`;
    let res;
 ...
 * @returns {RowDataPacket} Resultset from the query.
 */
async function getNewProductkod() {
    let sql = `CALL show_new_product_code();`;
    let res;
 ...
 * @returns {RowDataPacket} Resultset from the query.
 */
async function showStockOnProduct(produktkod) {
    let sql = `CALL show_stock_on_product(?);`;
    let res;
...
async function removeFromShelf(productId, shelfId, number) {
    ...
    sql = `CALL remove_from_stock(?, ?, ?);`;

    let res;

    res = await db.query(sql, [productId, shelfId, number]);
    //console.log(res);
    // console.info(`SQL: ${sql} got ${res.length} rows.`);
 ...
 * @async
 * @param {int} productId      Product to remove.
 * @param {int} shelfId      Shelf to remove.
 * @returns {RowDataPacket} Resultset from the query.
 */
async function hardDeleteFromStock(productId, shelfId) {
    let sql;

    sql = `CALL hard_delete_stock(?, ?);`;

    let res;

    res = await db.query(sql, [productId, shelfId]);
    //console.log(res);
    // console.info(`SQL: ${sql} got ${res.length} rows.`);
