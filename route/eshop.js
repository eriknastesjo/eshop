...
const eshop2 = require("../src/eshop2.js");
...
router.get("/eshop/edit/:produktkod", async (req, res) => {
    ...
    data.res = await eshop.showProductkod(produktkod);
    data.res2 = await eshop.showAllCategories();
    data.res3 = await eshop.showProductkodCategories(produktkod);
...
router.get("/eshop/create", async (req, res) => {
    ...
    data.res = await eshop.showAllCategories();
    data.newProductCode = await eshop.getNewProductkod();
...
router.post("/eshop/edit", urlencodedParser, async (req, res) => {
    ...
    for (const cat of categories) {
        if (cat) {
            await eshop.insertProductCategory(req.body.produktkod, cat);
        } else {
            console.log(req.body.produktkod);
            console.log(catIndex);
            // Behöver använda catIndex istället eftersom cat är undefined!
            await eshop.deleteProductCategory(req.body.produktkod, catIndex);
        }
        catIndex++;
    }
...
router.post("/eshop/create", urlencodedParser, async (req, res) => {
    // console.log(JSON.stringify(req.body, null, 4));
    // console.log(req.body.cat1)
    // console.log(req.body.cat2)
    // console.log(req.body.cat3)
...
router.get("/eshop/crud", async (req, res) => {
    ...
    data.res = await eshop.showProductOrderId();
 ...
 * /eshop/stock:
 *   get:
 *     summary: Display information stock
 *     description: CRUD - READ information stock
 */
router.get("/eshop/stock", async (req, res) => {
    let data = {
        title: `Show stock ${sitename}`,
    };

    data.res = await eshop.showInv();
    res.render("eshop/stock", data);
});

/**
 * EDIT STOCK
 * /eshop/stock/:produktkod:
 *   get:
 *     summary: Display information of order
 *     description: CRUD - READ information order
 */
router.get("/eshop/stock/edit/:produktkod", async (req, res) => {
    let produktkod = req.params.produktkod;
    let data = {
        title: `Show stock ${sitename}`,
    };

    data.res = await eshop.showShelfs();
    data.res2 = await eshop.showStockOnProduct(produktkod);
    data.produktkod = produktkod;

    res.render("eshop/stock-edit", data);
});
 ...
 * /eshop/stock
 *   get:
 *     summary: Add to stock
 *     description: CRUD - Add to stock
 */
router.post("/eshop/stock", urlencodedParser, async (req, res) => {
    console.log(JSON.stringify(req.body, null, 4));
    await eshop.addToShelf(
        req.body.produktkod,
        req.body.hylla,
        req.body.quantity
    );
...
/**
 * DELETE ROW FROM STOCK
 * /eshop/stock:
 *   get:
 *     summary: Add to stock
 *     description: CRUD - Add to stock
 */
router.post("/eshop/stock/delete", urlencodedParser, async (req, res) => {
    console.log(JSON.stringify(req.body, null, 4));
    await eshop.hardDeleteFromStock(
        req.body.produktkod,
        req.body.hylla
    );
    res.redirect(`/eshop/stock`);
...
/**
 * SHOW ALL PICKLIST
 * /eshop/picklist/:ordernummer:
 *   get:
 *     summary: Display information picklist
 *     description: CRUD - READ information picklist
 */
router.get("/eshop/picklist/", async (req, res) => {
    let data = {
        title: `Show picklist ${sitename}`,
    };

    data.res = await eshop2.showAllPlocklista();
 ...
 *     description: CRUD - READ information picklist
 */
router.get("/eshop/picklist/:ordernummer", async (req, res) => {
    let ordernummer = req.params.ordernummer;
    let data = {
        title: `Show picklist ${sitename}`,
    };

    data.res = await eshop2.showPlocklista(ordernummer);
    res.render("eshop/picklist-order", data);
});
 ...
 *     description: CRUD - Update order
 */
router.post("/eshop/picklist", urlencodedParser, async (req, res) => {
    console.log(JSON.stringify(req.body, null, 4));
    // ta bort från lagret
    const data = await eshop2.showPlocklista(req.body.ordernummer);

    for (const row of data) {
        await eshop.removeFromShelf(
            row.produkt,
            row.lagerhylla,
            row.antal_beställda
        );
    ...
    // ändra orderstatus till skickad
    await eshop2.updateKundOrder(
        req.body.ordernummer
    );

    res.redirect(`/eshop/picklist`);
});
